...
export default function NmapFlashcardsPage() {
  return (
    <main>
      <Suspense
        fallback={
...
function FlashcardsContent() {
  const totalCards = NMAP_STUDY.reduce((sum, d) => sum + d.topics.reduce((s, t) => s + t.flashcards.length, 0), 0);
  ...
  return (
    <div className="space-y-4 px-3 pt-4 sm:space-y-6 sm:px-0 sm:pt-0">
      ...
      <section className="rounded-2xl border border-zinc-800 bg-gradient-to-br from-orange-950/30 to-zinc-900 p-5 sm:rounded-xl sm:p-6">
        <h2 className="text-xl font-bold tracking-tight sm:text-2xl">Nmap Flashcards</h2>
      ...
      {/* All flashcards CTA */}
      <Link
        href="/nmap/flashcards/study"
        className="group flex items-center justify-between rounded-2xl border border-orange-800/30 bg-orange-950/20 p-4 transition-all active:bg-orange-900/30 sm:rounded-xl sm:hover:bg-orange-950/30"
      >
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-orange-900/40 text-sm font-bold text-orange-400">
            {totalCards}
          </span>
          <div>
            <span className="text-sm font-semibold text-zinc-200">Study All Flashcards</span>
            <p className="text-xs text-zinc-500">All topics combined</p>
          </div>
        </div>
        ...
        <section key={domain.key} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4 sm:rounded-xl sm:p-5">
          ...
          <div className="space-y-1.5">
            {domain.topics.map((topic) => (
              <Link
                key={topic.slug}
                href={`/nmap/flashcards/study?topic=${topic.slug}`}
