...
function FlashcardsContent() {
  return (
    <div className="space-y-4 px-3 pt-4 sm:space-y-6 sm:px-0 sm:pt-0">
      ...
      {/* All flashcards CTA */}
      <Link
        href="/linux/flashcards/study"
        className="group flex items-center justify-between rounded-2xl border border-emerald-800/30 bg-emerald-950/20 p-4 transition-all active:bg-emerald-900/30 sm:rounded-xl sm:hover:bg-emerald-950/30"
      >
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-emerald-900/40 text-sm font-bold text-emerald-400">
            {LINUX_STUDY.reduce((sum, d) => sum + d.topics.reduce((s, t) => s + t.flashcards.length, 0), 0)}
          </span>
          <div>
        ...
        <section key={domain.key} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4 sm:rounded-xl sm:p-5">
          ...
          <div className="space-y-1.5">
            {domain.topics.map((topic) => (
              <Link
                key={topic.slug}
                href={`/linux/flashcards/study?topic=${topic.slug}`}
